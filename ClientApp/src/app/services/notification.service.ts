...
@Injectable()
export class NotificationService extends EndpointService {
  constructor(
  ...
  updateAdvertisementSeenFlag(
    notificationId: number,
    advertisementId: string,
    flag: boolean
  ) {
    return this.http.put(
      "/api/advertisement/" + notificationId + "/" + advertisementId,
      flag
    );
  }
  ...
  queryAdvertisement(notification: Notification): Observable<Notification> {
    let endpointUrl = `${this.baseURL}/api/webscrap/query`;
    return this.http
      .post<Notification>(endpointUrl, notification)
      .pipe(retry(3));
  }
  ...
  calculateInterval(frequency: string) {
    ...
    switch (frequency) {
      case "Every hour": {
        tempInterval = 1 * 60 * 60 * 1000;
        break;
      } // 1s = 1000
      case "Every 4 hour": {
        tempInterval = 4 * 60 * 60 * 1000;
        break;
      }
      case "Daily": {
        tempInterval = 24 * 60 * 60 * 1000;
        break;
      }
    }
    return tempInterval;
